...
if __name__ == "__main__":
    ...
    parser = configargparse.ArgumentParser(
        description=__doc__,
        default_config_files=[nn_common.COMMON_CONFIG_FPATH],
    ...
    parser.add_argument(
        "--data_dpath",
        default=DATA_DPATH,
        help="Directory where the data to filter is located",
    )
    parser.add_argument(
        "--out_dpath",
        help="Filtered data output directory (default: data_dpath/../filtered/data_dname",
    )
    parser.add_argument("--maxISO", type=int, default=MAXISO, help="Maximum ISO value")
    args, _ = parser.parse_known_args()

    if args.out_dpath is None:
        args.out_dpath = os.path.join(
            args.data_dpath,
            "..",
            "filtered",
            f"ISO{args.maxISO}",
            utilities.get_leaf(args.data_dpath),
        )
